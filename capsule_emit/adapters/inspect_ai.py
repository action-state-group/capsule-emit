...
call, one per sample's terminal state.
...
``ModelEvent.call``). This adapter reads that transcript after a run
...
def seal_eval_log(
    ...
    **core_kwargs: Any,
    ...
    Per sample: one record per ``ModelEvent`` in ``sample.events`` (in
    order, chained), then one terminal record chained to the last of those
    (or unchained if the sample has no model-call event at all). Returns
    ``{"<sample_id>#<epoch>": [EmitResult, ...]}`` in the same order.
    ...
    for sample in log.samples or []:
        ...
        for event in sample.events:
            ...
            if call is None or call.request is None:
                # Inspect recorded no raw call at all for this event -- there
                # is nothing honest to digest, so nothing is sealed for it.
                continue
