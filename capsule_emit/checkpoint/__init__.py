...
**Since 0.5.0, ``seal()``/``received()`` wire this in by DEFAULT** (see
...
required. Disable per-call with ``witness=False`` or everywhere with the
``CAPSULE_WITNESS=off`` env var.
...
capsule_emit`` alone, or a single below-cadence ``seal()``/``received()``
call) loads an MMR module or pulls in a network dependency. A caller who
wants direct control over cadence, signing keys, or a non-default
Transparency Service can still
