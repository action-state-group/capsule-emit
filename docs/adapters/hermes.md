...
set `CAPSULE_WITNESS=off` (the proof above does) — offline `verify` checks the
same things either way; what you turn off is the outside reference. The
`operator` and `developer` you pass to the emitter seal into the hash-chained
record permanently — use a
