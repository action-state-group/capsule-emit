...
Chain Navigation table. `demo.py`'s own `_permalink`/`_bundle_permalink` helpers build it
directly from the live-anchored ledger (`examples/goose-capsule/demo.py`, step 9 of the run —
see `evidence/run-transcript.md` for the full output), after every capsule in the chain has
already passed `verify().ok` (step 6) — a presenter can't hand out a link for a capsule that
failed local verification.

**A standalone `capsule-emit permalink` CLI subcommand does not exist yet on this branch** —
only `demo.py`'s in-process helpers do. `capsule_emit.permalink` (a `--check`/`--reveal`-style
CLI built on top of the same `build_url()` logic) is tracked as a follow-up once this branch
rebases onto a main that has the module; don't run `capsule-emit permalink ...` expecting it to
work today.

**Revealed links (`--reveal <artifact>`) are not available yet either** — that mode depends on
[aac-disclosure-envelope], a separate, not-yet-built disclosure-envelope format change. Only
withheld links (like the one above) ship in this build.
